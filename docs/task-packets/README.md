...
1. Check the packet's **Start gate** and its predecessor's handoff report. `STATUS: done` alone is not a gate.
2. Give one agent one packet. Parallel agents share only declared public contracts and never edit the same owned files.
3. The agent records its contract ledger, implements its bounded slice, and returns [the handoff report](protocols/handoff.md).
4. For CR, supply a new, read-only agent the exact packet path, predecessor report, implementation report, diff/commit, test output, and [the CR protocol](protocols/code-review.md).
5. The lead resolves findings. An integration packet is the normal point where a phase's public contract is published.

`partial` and `blocked` are useful outcomes: never dispatch past an unmet start gate. Implementers may not edit `docs/`, `prompts/`, or `DESIGN.md`.
