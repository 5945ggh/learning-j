...
> 标注 **[不可推迟]** 的字段与约束，须在首次持久化该阶段真实学习数据、建立受保护 schema 基线或对外发布前落实；在此之前明确指定的可丢弃开发／测试库可按 ADR-042 重建，不改变运行时的引用、偏移、授权、事务与幂等约束。
...
> 开发期 schema 基线与正式数据保护边界见 [ADR-042](adr/042-development-schema-baseline.md)；不表示已建立该基线或实现升级流程。
...
- **词形作用域键**：`scope_form_key` 是确定性、非空的作用域键。Lexeme 级使用 `lexeme:`；具体词形使用 `form:` 加该原词形的 Unicode NFC 结果。具体词形不得 trim，也不做大小写、全角／半角或其他宽窄折叠；空具体词形直接拒绝。原始输入仍单独保存。KE 与 `LexemeKnowledgeDecision` 必须使用同一派生规则；唯一性与查询索引使用 `(lexeme_id, scope_form_key)`，不得依赖 NULL 的唯一性行为。
- **作用域序号与索引**：导入结果在 `(import_run_id, lexeme_id, scope_form_key)` 内唯一；`LexemeKnowledgeDecision.decision_seq` 在 `(lexeme_id, scope_form_key)` 内单调递增且唯一。读取当前决定使用该作用域的有序索引取最新序号，不扫描全库。
...
| KnownEvidence、LexemeKnowledgeDecision、KnownEvidenceRetraction、已发布导入结果、Occurrence 的提取内容与来源、已提交提取结果、KnowledgePointRetentionDecision、ReviewItemAdmissionDecision、ReviewEvent | 追加保存；纠错或重跑记录新的事实，不重写来源 |
| 会话阶段、任务状态、当前指针、访问时间 | 直接更新；无需为每次进度变化创建实体版本 |
| KP `default_retention`／canonical_id | 更新当前值，同时追加用户意愿／合并决策事件；抽取不可覆盖用户意愿。KP 默认复习策略的用户变更追加 `KnowledgePointRetentionDecision`；仅已有用户决定后的同值重复设置不追加，首次明确接受初始化默认值仍追加决定 |
| Occurrence `retention_override` 与用户确认 | 会话内的局部选择由 `SessionConfirmation` 追加保存；知识库直接修改才追加紧凑的 `OccurrenceRetentionDecision`。两者与提取内容分离，不引入通用操作日志；局部值及所依赖 KP 策略使用版本检查，确认加入与建卡原子提交 |
...
| `scope_form_key` | string | **[不可推迟]** 由 `conjugated_form` 按 §0 规则派生：空值为 `lexeme:`，具体词形为 `form:` + NFC 原词形；用于作用域索引、当前裁定和各自约束，不由调用方自由填写 |
...
导入行按 `(import_run_id, lexeme_id, scope_form_key)` 唯一，空词形通过明确的 scope key 表达唯一性，不依赖 SQLite 的 NULL 唯一行为。多个输入项映射到同一行时保留关联；不同运行可追加证据，查询不因重复导入累加置信度。用户断言由下述 known 决定与 KE 同一事务产生，不能通过访问解析页或确认 srs 间接产生。

#### 用户当前判断与来源撤回

- `LexemeKnowledgeDecision(lexeme_id, conjugated_form?, scope_form_key, analyzer_dict_version, decision, evidence_id?, decision_seq, operation_key)` 追加保存。decision 为 `known` / `unknown` / `clear`；known 必须引用同次提交、同目标同作用域的 user_asserted KE，其余 evidence_id 为空。每个作用域具有单调 decision_seq 与预期版本检查，重试幂等；analyzer_dict_version 记录作出目标判断时的身份口径，迁移不改写旧裁定。
- 每个 `(lexeme_id, scope_form_key)` 只取最新决定。known 是用户正向裁定；unknown 是用户当前明确否认，压过该作用域的导入与 SRS 估计，但不伪造负向 KE、不删除复习历史；clear 清除该作用域的人工覆盖，回到其他来源求值，**不重新启用旧 user_asserted**。用户来源只有被当前 known 决定引用的有效 KE 可以贡献。最新决定使用 `(lexeme_id, scope_form_key, decision_seq)` 有序索引取得，不扫描全库。
...
3. 当前写入路径不支持把 ReviewItem 换绑到新的 Occurrence；若历史数据或迁移记录存在换例句痕迹，资格校验须核对新提示与原累计目标是否一致。不一致时仅停用当前状态的跨层信号，旧事件仍保留；此历史兼容防线**不清零、回滚或暂停正常 SRS 排程**。
...
| `default_retention` | enum | **[不可推迟]** `srs` / `reference`。KP 默认复习策略，供 Occurrence 继承；不表示已有卡片或逐例句建卡授权。无决定记录时初始化为 `srs` |
...
KP 的 `default_retention` 对用户称为“默认复习策略”。`srs` 允许已获逐例句授权的复习继续排程，`reference` 暂停继承该策略的已有复习项；两者都不是对全部历史或未来 Occurrence 的建卡授权，也不表示卡片数量或学习进度。两者都保留 KP、Occurrence、检索、聚合与记忆注入；知识点的存在不等于“已学会”。

Occurrence 另有局部 `retention_override` 决定，取值为 `inherit` / `srs` / `reference`。`inherit` 使用其原始 `kp_id` 对应 KP 的当前 `default_retention`；`srs` 与 `reference` 是用户对具体例句或讲解实例的明确覆盖。有效意愿由后端求值并带来源，不把局部选择无提示地写回 KP。

**意愿值与用户是否确认分开**：`retention_override = inherit` 本身不说明用户是否处理过该 Occurrence。会话内以 §4.2 的确认记录判断，知识库动作保留逐 Occurrence 的用户决策记录，不以当前局部覆盖或 KP 是否有用户决定推导。用户明确沿用 KP 策略也必须确认具体 Occurrence；当次有效值为 `srs` 时，确认与 ReviewItem 的创建／复用原子提交。确认时为 `reference` 且没有 ReviewItem 的 Occurrence，后续仅因 KP 改为 `srs` 不会自动建卡，仍须用户明确加入。

1. 新 KP 创建时使用 `default_retention = srs` 作为尚未有用户决定的初值；Occurrence 默认 `retention_override = inherit`。该初值不授权建立 ReviewItem。若用户首次明确接受该默认值，即使值仍为 `srs`，也追加一条 `KnowledgePointRetentionDecision`；只有已有决定且新值相同的重复设置才不追加。
2. 用户在学习会话确认中选择 Occurrence 时，记录 Occurrence 级决定；若需要同时开放整个 KP，必须明确选择该附加操作。知识库可以直接修改 KP 意愿。两类决定均追加可审计事件，重跑抽取不得覆盖用户决定。
3. 后续抽取保留已有 KP 的 `default_retention`，新 Occurrence 以 `retention_override = inherit` 初始化，不复制旧 Occurrence 的局部决定或加入授权。标签可以并集补充；自动处理不覆盖人工 `display_form`。
   KP 为 `reference` 时，继承状态的 Occurrence 可在确认界面灰显或默认不选，但用户可明确选择局部 `srs`。KP 意愿变化不删除或退役卡片；只影响当前有效意愿继承该 KP 的 ReviewItem。
4. 会话中的候选按本次成功 ExtractionRun 的 KP 折叠展示，但卡片选择以 Occurrence 为粒度。是否完成本次确认由 §4.2 的 SessionConfirmation 决定，不以是否存在 KP 决定记录作为解析队列或会话退出条件。
...
- `shape = entity` 的 payload 必须含 `entity_type` 与 `entity_label`；`anchor` 为按 NFC、空白规范化后的 `entity:<entity_type>:<entity_label>`，同键直接复用 KP，不以语义近邻自动合并。
- `anchor` 必须由 payload 按记录的版本确定性派生；槽位 id 不参与身份。复用已有 pattern KP 时，后端按结构位置（从左到右的槽位序号及其 category/form）把候选槽位 ID 映射到既有槽位 ID；结构不兼容或映射不唯一时拒绝该次 pattern 出现，不保存候选自定义 ID。
- 展示优先使用人工 `display_form`，否则从 payload 渲染。被保存的导出产物记录渲染版本；当前 UI 渲染器可正常维护，不要求每次视觉修复新增永久函数。

#### KnowledgePointRetentionDecision（KP 默认策略决定）

KP 当前 `default_retention` 之外，用户对默认策略作出明确决定时追加一条决定记录，供后续 `inherit` 确认引用：

| 字段 | 类型 | 说明 |
|---|---|---|
| `decision_id` | uuid | 决定记录身份 |
| `kp_id` | FK | 目标 KP |
| `previous_default_retention` | enum | 决定前的 `srs` / `reference` |
| `default_retention` | enum | 决定后的 `srs` / `reference` |
| `source` | enum | `knowledge_base` / `session_confirmation` |
| `decision_seq` | int | 同一 KP 内单调递增；与 `kp_id` 组成有序唯一索引 |
| `operation_key` | string | 幂等键 |
| `created_at` | timestamp | 决定时间 |

没有决定记录时，KP 的 `default_retention = srs` 是初始化默认值，不可作为 `inherited` 的授权来源。用户首次明确接受默认值也视为决定，追加 `previous_default_retention = srs`、`default_retention = srs` 的记录；已有决定且值相同的重复设置不产生新记录。每条决定提交时必须在同一事务中把 KP 的 `default_retention` 更新为该最新值；不存在独立的“恢复未确认”写路径。`decision_id` 可被 `SessionConfirmation` 在 `decision_source = inherited` 时引用；它不记录每次页面操作，也不取代会话确认本身。
...
| `slot_bindings` | json | 槽位实例绑定，键为**已复用 KP 的规范槽位 id**（如 "N1"），值为 `span_id`（引用 `spans` 表）。候选自定义槽位 id 只用于提取阶段映射，不得持久化；`shape` 为 `pattern` 时按模式槽位填充，其他 shape 时为空对象 `{}` |
| `salience` | enum | **[不可推迟]** `primary` / `secondary`。用于候选排序与用户决定后的 queued 首次准入优先级；不决定 KP 的存在或复习意愿（ADR-019） |
| `retention_override` | enum | **[不可推迟]** `inherit` / `srs` / `reference`。Occurrence 的局部复习意愿覆盖；`inherit` 使用原始 KP 的当前 `default_retention` |
...
5. `slot_bindings` 的值为 `span_id`（原文 surface 可从 Span 取，不冗余存储）。
6. 候选 pattern 复用已有 KP 时，槽位绑定键先按结构位置映射到该 KP 的规范槽位 ID，再执行完整性校验；映射不唯一、category/form 不兼容或键集合不完整时拒绝该次出现并记录 unresolved_patterns。
...
| `parent_session_id` | FK? → StudySession | 关联新会话的前驱；普通会话为空。非空时必须同时填写 `source_analysis_revision_id`；MVP 仅用于“继续研究”关联，不合并生命周期、确认或 ReviewItem |
| `source_analysis_revision_id` | FK? → AnalysisRevision | 关联新会话继续研究时所依据的不可变 revision；有 `parent_session_id` 时必填且必须属于该前驱会话，不作为新会话当前文档指针 |
...
- 已完成会话保留文档、对话和来源，可从学习记录访问。继续研究时创建新的关联会话，同时写入 `parent_session_id` 与 `source_analysis_revision_id`；MVP 的关联语义仅是同一来源句／revision 的后续研究，不修改或复用原会话的确认、Occurrence 或 ReviewItem。
...
| `kp_id` | FK | 必须等于该 `occurrence_id` 的 `occurrence.kp_id`，且该 Occurrence 必须属于该 run；不得只因 KP 出现在 run 中而通过 |
| `effective_retention_at_confirmation` | enum | 本次确认时的有效 `srs` / `reference` 策略快照 |
| `retention_override_at_confirmation` | enum | 本次确认的局部覆盖值 `inherit` / `srs` / `reference`；与有效意愿快照分开 |
...
| `decision_id` | FK? → KnowledgePointRetentionDecision | 仅 `decision_source = inherited` 时填写，指向所沿用的 KP 决定；`decision_source = user` 时必须为 `NULL`，直接确认由本行承载，不另建 Occurrence 决定事件 |
| `confirmed_at` | timestamp | |

确认组合只有以下三种：`(decision_source = user, retention_override_at_confirmation = srs, decision_id = NULL)`、`(user, reference, NULL)`、`(decision_source = inherited, retention_override_at_confirmation = inherit, decision_id = current_kp_decision_id)`。前两种的 `effective_retention_at_confirmation` 必须分别等于 `srs`／`reference`；后一种必须引用同一原始 `kp_id` 的当前有效 `KnowledgePointRetentionDecision`，其 `default_retention` 必须等于快照，并通过 KP 与 Occurrence 版本检查。没有 KP 决定记录时不得使用 `inherited`；default 初值不能作为用户选择的替代。Occurrence 的局部 `srs` / `reference` 选择始终记录为 user，且 `decision_id` 必须为 `NULL`。

无确认记录的 `retention_override = inherit` 是尚未确认；存在合法 `inherited` 确认记录的 `inherit` 才表示用户明确沿用。`reference` 只作参考也是一次有效确认，不表示未处理。历史确认不授予其他 Occurrence 建卡权限，后续 KP 策略变化不改写当时的局部覆盖值、有效值或 ReviewItem 引用。

记录追加保存；同一会话、run、Occurrence 的最后一次明确确认是当前确认结果。继承默认策略不自动完成会话；可提供列明具体 Occurrence 与结果的“一并沿用并完成”。确认一个新选择时，局部覆盖、必要的 KP 决定、SessionConfirmation 及所需 ReviewItem 的创建／复用同一事务提交；任何一步失败均不提交“已加入学习”，重试不重复创建。会话结束后知识库修改不回写这些快照。

`SessionConfirmation` 本身就是会话内“用户是否处理并加入该 Occurrence”的审计事实，不另写一条重复的 Occurrence 决策事件。知识库直接修改局部覆盖时使用下方定义的 `OccurrenceRetentionDecision`；相同值的重复写入不产生事件。新建 ReviewItem 时另有 §7.0 定义的最小 `ReviewItemAdmissionDecision` 授权事实；它不是通用操作日志，正常会话来源只反向引用本行。该记录不保存页面状态、完整请求正文或重复的 Occurrence 内容。

最后一项确认与会话完成在同一提交边界；等待配额不阻止会话完成。若确认期间全局意愿已被别处更新，按版本检查提示刷新，不静默覆盖。KP 合并影响正在确认的结果时同样刷新，不复制意愿。

#### OccurrenceRetentionDecision（知识库局部覆盖决定）

| 字段 | 类型 | 说明 |
|---|---|---|
| `decision_id` | uuid | 决定记录身份 |
| `occurrence_id` | FK | 目标 Occurrence |
| `previous_retention_override` | enum | 决定前的 `inherit` / `srs` / `reference` |
| `retention_override` | enum | 决定后的局部覆盖值 |
| `source` | enum | `knowledge_base` |
| `operation_key` | string | 幂等键 |
| `created_at` | timestamp | 决定时间 |

该记录只用于知识库直接修改局部覆盖；会话确认由 `SessionConfirmation` 承载。相同值的重复设置不追加记录。它不授予建卡权限；独立的知识库“加入复习”操作仍须为新 ReviewItem 建立 `ReviewItemAdmissionDecision(source = knowledge_base)`。
...
- `AgentRun`：关联 session_id，记录 `purpose = generation | discussion`、`status = queued | running | paused | done | failed | cancelled`、`pause_reason?`（MVP 为 `budget_exhausted`）、`resume_of_agent_run_id?`、provider/model/prompt_version、请求标识、usage、输入／输出文档 revision、注入的 KP/note ID 与必要版本或摘要。初始 generation 是一次运行；每次用户发送消息产生 discussion 运行，运行内部可多次调用工具。预算暂停保留原运行；用户明确续接时新建运行并指向 `resume_of_agent_run_id`。
...
提取结果使用 run 级原子提交闸门：提交事务先验证该 run 仍为 `running` 且通过调用方携带的预期 run/version 检查，再在同一短事务中写入全部成功产物、失效记录并将 run 标为 `done`。同一 run 的并发提交至多一方成功；`done` 的重放直接返回已提交的结果与产物 ID，不再插入。提交前崩溃由事务回滚，不留下半批结果；提交后响应丢失时，重试读取原 `done` 结果。不得新增 `(run, kp, start, end)` 内容唯一约束：它会忽略小节来源等学习事实；不同成功 run 可追加合法 Occurrence。
...
### 7.0 ReviewItemAdmissionDecision（建卡授权事实）

每个新建 ReviewItem 都需要一个只表达“明确加入”的最小授权事实；正常会话中的明确 `srs` 确认由 `SessionConfirmation` 承载并关联该授权，知识库直接加入或退役后重新建卡则由本实体承载。退役后重新建卡可以不经过新会话，因此授权还需区分 `explicit_readd`：

| 字段 | 类型 | 说明 |
|---|---|---|
| `admission_decision_id` | uuid | 授权事实身份 |
| `occurrence_id` | FK | 被明确加入的 Occurrence |
| `target_review_item_id` | FK | 本次授权创建的 ReviewItem |
| `effective_retention` | enum | 授权时的有效值，必须为 `srs`；`explicit_readd` 可在当前 `retention_override` 已是 `srs` 时不改写该字段 |
| `source` | enum | `session_confirmation` / `knowledge_base` / `explicit_readd` |
| `session_confirmation_id` | FK? | `source = session_confirmation` 时必填并指向对应确认；`explicit_readd` 在由会话触发的重建场景可选填以追溯会话；普通 `knowledge_base` 加入及无会话的 `explicit_readd` 必须为空 |
| `supersedes_review_item_id` | FK? | `explicit_readd` 时指向被退役的旧 ReviewItem；普通首次建卡为空 |
| `operation_key` | string | 幂等键 |
| `created_at` | timestamp | 授权时间 |

`source = session_confirmation` 且本次创建新 ReviewItem 时，该记录与建卡同一事务提交，不是第二条用户操作日志；若只是复用已有非 retired ReviewItem，则由 `SessionConfirmation` 单独承载，不追加本授权事实，也不初始化或丢弃该卡的状态与进度。`source = knowledge_base` 表示知识库内的明确逐 Occurrence 加入，`session_confirmation_id` 必须为空；`source = explicit_readd` 只允许在同一 Occurrence 的旧 ReviewItem 已 `retired` 时创建新 ReviewItem，若由会话内的再次加入／重建动作触发可填写 `session_confirmation_id`，普通知识库再次加入为空。仅重复设置同一局部覆盖值不创建该记录，也不改变 `OccurrenceRetentionDecision` 历史。每个新 ReviewItem 恰好对应一条本授权事实；`target_review_item_id` 唯一且其 `occurrence_id` 必须与本记录一致。
...
- 用户明确选择 Occurrence 加入学习时立即创建或复用 ReviewItem；若创建新项，则恰好写入一条对应的 `ReviewItemAdmissionDecision`，并与建卡同一事务提交。每日配额不足时为 `queued`，按 salience 优先、同级 FIFO 转为 `active`。默认值或 KP 的 `srs` 意愿不单独授权创建；复用既有非 retired 项不追加授权、不初始化或丢弃其状态与进度，但仍可依据有效策略正常转换状态。
- 同一 Occurrence 默认至多一条非 retired ReviewItem；同一 KP 下不同 Occurrence 可以各自拥有 ReviewItem。未来不同卡片模板可扩展为多个 ReviewItem，但必须有明确用户操作。
- 有效策略为 `reference` 时，既有且未 retired 的对应 ReviewItem 进入 `paused`，不进入到期队列；保留既有 ReviewState 与 ReviewEvent（尚未首评则两者均不存在）。已 retired 项保持 retired，不能因参考确认复活为 paused。reference 且无卡的 Occurrence 不建卡；新建 ReviewItem 仅由明确的 `srs` 加入授权产生，初始状态为 `queued`，配额只处理其首次准入。复用既有非 retired 项不初始化或丢弃其状态与进度，仍可依据最新有效策略正常在 `paused`、`active`、`queued` 之间转换。KP 意愿变化只影响继承该意愿的 Occurrence，明确 `srs` 覆盖的 Occurrence 由用户决定是否暂停。
...
- 已退役 ReviewItem 不恢复；用户若要再次加入同一 Occurrence，必须明确执行“再次加入复习”，创建 `source = explicit_readd` 的 `ReviewItemAdmissionDecision` 并新建 ReviewItem。该动作不要求改变当前 `retention_override`，也不因同值设置生成 `OccurrenceRetentionDecision`；它只通过授权事实表达新的建卡决定。会话内触发的重建可以在授权事实中填写可选 `session_confirmation_id` 以追溯来源；普通知识库加入保持为空。

每日新卡配额以用户配置的 IANA `schedule_timezone` 计算本地 `quota_day`，不以 UTC 日界隐式切换；未配置时使用设备当前 IANA 时区并在排程设置中可见。跨日只重置可用配额：已准入但尚未首评的 `active` 项保持 active，不重新消耗新卡配额；仍为 `queued` 的未准入项跨日保留并按下一日的 salience/FIFO 继续等待。排程服务必须持久化本次配额结算的 `quota_day` 与幂等键，重启或重复运行不得重复扣减。
...
ReviewEvent 追加保存：`review_item_id`、该卡单调 `event_sequence`、当时的 `occurrence_id`、`reviewed_at`、评分、算法版本及重放所需的排程参数／结果。`(review_item_id, event_sequence)` 唯一；ReviewState 带覆盖到的事件序号与单调版本，供重建和资格摘要校验。同一次复习提交有幂等键；**首条 ReviewEvent 与该 ReviewState 的建立**、以及后续事件写入与当前状态更新，均在同一事务内完成。当前流程不换绑 Occurrence；若旧数据带有换例句记录，只用于历史资格核对，不删除既有复习事件，也不伪造评分。
...
3. 有效策略为 reference 的 Occurrence 没有 active ReviewItem；既有非 retired 卡片转为 paused 并保留复习历史，retired 项不复活，新建项仍按确认与排程规则处理。
...
13. pattern Occurrence 的持久化 `slot_bindings` 键集合等于模式槽位集合；其他 shape 为空对象。模型输入输出使用 `slot_surfaces`，候选槽位 ID 到规范槽位 ID 的映射由后端完成。
...
18. 任意新建复习项均有且仅有一条针对具体 Occurrence 的明确加入决定：正常会话通过 `SessionConfirmation` 与对应 `ReviewItemAdmissionDecision(source = session_confirmation)`，知识库直接加入通过 `ReviewItemAdmissionDecision(source = knowledge_base)`，退役后重建通过 `ReviewItemAdmissionDecision(source = explicit_readd)`，并与建卡原子提交；`target_review_item_id` 唯一且与 occurrence 一致。复用既有项不新增授权、不初始化或丢弃状态与进度，但仍可依据有效策略正常转换；default 或 KP 策略变化不构成授权。首次准入排程须满足配额，未准入项为 queued；已准入项的暂停恢复不重复消耗配额、不清除进度。
19. 提取、确认、Agent 工具副作用与复习提交各自原子且幂等；提取同一 run 并发至多一方发布，done 重放返回原结果；`SessionConfirmation.kp_id = occurrence.kp_id`，确认组合仅为两种 user 局部值或一种 inherited + 当前 KP 决定；失败不得留下宣称成功的部分产物。
...
23. SRS 跨层信号有实际评分 Occurrence 与完整累计状态的资格依据；合并或历史换例句记录不能转授旧评分，不合格状态不影响原有排程。
...
- 最小索引访问路径：KE(lexeme_id, scope_form_key, source, observed_at, id)、KE(import_run_id, lexeme_id, scope_form_key)、decision(lexeme_id, scope_form_key, decision_seq)、retraction(evidence_id)／retraction(import_run_id)、import_entry(run_id, entry_ordinal)、material_count(material_id, sidecar_id, lexeme_id) 与反向 (lexeme_id, material_id, sidecar_id)、KP lexical anchor 双向关联、ReviewEvent(review_item_id, event_sequence)、eligibility(lexeme_id, review_item_id)。`scope_form_key` 是 lexeme_id 下的作用域；KE 的跨运行事实仍可追加，导入唯一性另由 `(import_run_id, lexeme_id, scope_form_key)` 约束，决定唯一性另由 `(lexeme_id, scope_form_key, decision_seq)` 约束。物理列顺序按实际谓词与查询计划验证，外键不替代必要索引。
...
| 历史换例句记录、KP 合并／撤销、lexical_anchors 改变 | 原目标和新目标、受影响卡片资格；不能只处理新 canonical |
