...
- 素材工作区：以原文或播放器为主，配一个可折叠的算法／词典区与一个紧凑的当前材料会话列表。三栏不是必须的。
...
## 开放问题
- 唯一的未决事项索引是 `docs/LearningJ-plan-v5.md` §15。它跟踪主页默认内容、工具栏／查词细节、onboarding 与 Agent 运行默认值。本文档确定的导航标签（“主页”“解析队列”“学习中心”）与“导航不触发提取”已定；主页具体聚合仍由 §15.23 管理。不要把已定的标签或失败出口重新打开，也不要把旧入口名当作现行名称。
